  ...
  componentDidMount() {
    ...
    renderTripActionButton = () => {
      ...
      const renderGearRequestButton = () => {
        if (this.state.editingGear) return <div className="doc-button" onClick={saveGearRequest} role="button" tabIndex={0}>Save your gear request</div>;
      ...
      switch (this.state.role) {
        case 'OPO':
          return <div className="doc-button" onClick={goToTripPage} role="button" tabIndex={0}>View trip as OPO staff</div>;
        case 'LEADER':
          return (
            <>
              <div className="doc-button" onClick={goToTripPage} role="button" tabIndex={0}>Manage your trip</div>
        ...
        case 'MEMBER':
          return (
            <>
              <div className="doc-button alarm" onClick={goToTripPage} role="button" tabIndex={0}>I can't go</div>
              {renderGearRequestButton()}
            </>
          );
        case 'PENDING':
        ...
        case 'NONE':
          return <div className="doc-button" onClick={signUpForTrip} role="button" tabIndex={0}>Sign up</div>;
        default:
          return <div className="doc-button" onClick={signUpForTrip} role="button" tabIndex={0}>Sign up</div>;
      }
    }
            ...
            {this.state.viewMode === 'action'
              ? (
                <>
                  ...
                  <Stack size={25} />
                  <RequestGear trippeeGear={this.props.trip.trippeeGear} selfGear={this.state.requestedGear} isEditing={this.state.editingGear} updateGear={update => this.setState({ requestedGear: update })} loading={this.state.actionPending} />
                  <Stack size={25} />
